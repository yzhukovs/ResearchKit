...
- (void)viewDidLoad
{
    ...
    if (!self.picker.title) {
        self.title = NSLocalizedStringFromTableInBundle(@"picker.navigation.title",  @"Data donation", [NSBundle bundleForClass:GMImagePickerController.class], @"Navigation bar default title");
    } else {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([assetsFetchResult count] > 0) {
        ...
        if ([assetsFetchResult count] > 1) {
            ...
            PHAsset *assets = assetsFetchResult[1];
            [self.imageManager requestImageForAsset:assets
        ...
        if ([assetsFetchResult count] > 2) {
            ...
            PHAsset *assetss = assetsFetchResult[2];
            [self.imageManager requestImageForAsset:assetss
